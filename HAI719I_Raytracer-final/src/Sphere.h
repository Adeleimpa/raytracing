...
struct RaySphereIntersection{
    ...
    float t;
    float theta,phi; // spheric coordinates of our object
    Vec3 intersection;
...
public:
    ...
    RaySphereIntersection intersect(const Ray &ray) const {
        ...
        float b = 2 * Vec3::dot(ray.d, (ray.o - m_center));
        float c = (ray.o - m_center).squareNorm() - (m_radius*m_radius);

        float delta = (b*b) - (4*a*c);
        if(delta >= 0. and 2*a != 0){
            // find roots for the unknown t
            float t1 = (-b - sqrt(delta)) / (2*a);
            float t2 = (-b + sqrt(delta)) / (2*a);

            // take the root that is the closest to the origin of the ray
            if(t1 < t2 and t1 >= 0){
                intersection.intersectionExists = true;
                intersection.t = t1;
                intersection.intersection = ray.o + t1 * ray.d;
                intersection.secondintersection = ray.o + t2 * ray.d;
                Vec3 dist_center_intersection = intersection.intersection - m_center;
                dist_center_intersection.normalize();
                intersection.normal = dist_center_intersection;
            }else if(t2 < t1 and t2 >= 0){
                intersection.intersectionExists = true;
                intersection.t = t2;
                intersection.intersection = ray.o + t2 * ray.d;
                intersection.secondintersection = ray.o + t1 * ray.d;
                Vec3 dist_center_intersection = intersection.intersection - m_center;
                dist_center_intersection.normalize();
                intersection.normal = dist_center_intersection;
