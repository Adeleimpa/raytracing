...
public:
    ...
    void build_bounding_box() {
        ...
        Vec3 epsilon(0.0001, 0.0001, 0.0001);
        bounding_box.min_corner = Vec3(min_x, min_y, min_z) - epsilon;
        bounding_box.max_corner = Vec3(max_x, max_y, max_z) + epsilon;
        bounding_box.setDimensions();
    }
    ...
    RayTriangleIntersection intersect( Ray const & ray ) const {
        ...
        int nr_triangles = this->triangles.size();
        RayTriangleIntersection rti;

        for(int i = 0 ; i < nr_triangles; i++){ // go through each triangle of the mesh
            Vec3 corner0 = this->vertices[this->triangles[i][0]].position;
            Vec3 corner1 = this->vertices[this->triangles[i][1]].position;
            Vec3 corner2 = this->vertices[this->triangles[i][2]].position;
            float triangleScaling = 1.000001; // to avoid precision problem
            corner0 *= triangleScaling;
            corner1 *= triangleScaling;
            corner2 *= triangleScaling;
            rti = Triangle(corner0,corner1,corner2).getIntersection(ray);
