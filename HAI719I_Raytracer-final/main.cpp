...
// make clean && make && ./raytracer (to recompile and avoid errors, un truc commeca) (terminal?)
...
void ray_trace_from_camera() {
    ...
    camera.apply();
    Vec3 pos , dir;
    unsigned int nsamples = 10; // TODO back to 50
    std::vector< Vec3 > image( w*h , Vec3(0,0,0) ); // color of pixels
    int count = 0;
    for (int y=0; y<h; y++){
        for (int x=0; x<w; x++) {
            count++;
